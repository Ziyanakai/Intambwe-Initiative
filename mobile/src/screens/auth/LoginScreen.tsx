    ...
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.sub}>Sign in as {role === 'PARENT' ? 'Parent / Caregiver' : 'Doctor / Specialist'}</Text>
          ...
          <Button label="Sign In" onPress={login} loading={loading} style={{ marginTop: 8 }} />

          <TouchableOpacity onPress={() => navigation.navigate('Register', { role })} style={styles.register}>
            <Text style={styles.registerText}>Don't have an account? <Text style={{ color: colors.green600, fontWeight: '700' }}>Register</Text></Text>
          </TouchableOpacity>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  register: { alignItems: 'center', marginTop: 20 },
  registerText: { fontSize: 14, color: colors.ink2 },
})
